<template>
  <div>
    ...
    <el-card class="box-card">
      ...
      <!-- 用户列表区 -->
      <!-- border 加边框线  stripe单行单色 -->
      <el-table :data="userList" border stripe>
        <!-- type="index" 表示索引列 -->
        <el-table-column type="index"></el-table-column>
        <!-- label  当前列标题  prop 当前列对应的数据： 数据库返回的字段值 -->
        <el-table-column prop="username" label="姓名"></el-table-column>
        <el-table-column prop="email" label="邮箱"></el-table-column>
        <el-table-column prop="mobile" label="电话"></el-table-column>
        <el-table-column prop="role_name" label="角色"></el-table-column>
        <el-table-column prop="mg_state" label="状态">
          <!-- 定义作用域插槽，通过scope拿到当前作用域数据 -->
        ...
        <el-table-column label="操作" width="180px">
          <template slot-scope="scope">
            ...
            ></el-button>
            <!-- 分配角色按钮 -->
            <!-- effect 提示文字的背景色 placement 显示位置 content 显示文本 enterable 鼠标进入图标是否自动隐藏-->
            <el-tooltip
              class="item"
              effect="dark"
              content="分配角色"
...
export default {
  // 当前组件的私有数据
  data() {
    // 验证邮箱的规则 rules,校验规则， value 要校验的值， cb回调函数
    // 通过 validator在 校验规则中将其指定
    var checkEamil = (rules, value, cb) => {
      // 验证邮箱的正则表达式
  ...
  methods: {
    async getUserList() {
      // this.$http.get('请求路径', {请求参数}) 发起get请求
    ...
    handleSizeChange(newSize) {
      ...
      // 请求参数发生改变，需发送新的请求获取数据，得到最新的结果，将其展示
      this.getUserList()
    },
    // 监听页码值改变的事件,当前显示的页码值
    ...
    async removeUserById(id) {
      // 弹框询问用户，是否删除数据,如果用户确认删除，则返回值为字符串 confirm
